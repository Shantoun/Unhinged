...
def init_session():
    ...
    if "user_email" not in st.session_state:
        st.session_state.user_email = "ahaddadproject@gmail.com"
    ...
    if "_rerun_fix_used" not in st.session_state:
        st.session_state._rerun_fix_used = False

    # defaults used by tabs
    if "convo_min_mins" not in st.session_state:
        st.session_state.convo_min_mins = 5
    if "convo_min_messages" not in st.session_state:
        st.session_state.convo_min_messages = 3
    if "join_likes_comments" not in st.session_state:
        st.session_state.join_likes_comments = False
...
def safe_run(app_fn):
    try:
        ...
        if not st.session_state._rerun_fix_used:
            st.session_state._rerun_fix_used = True
            st.rerun()

        # still failing after auto-rerun -> show real error
        st.session_state._rerun_fix_used = False
        st.error("App hit an error and couldn't auto-recover.")
        st.code("".join(traceback.format_exception(type(e), e, e.__traceback__)))
        st.stop()


# ---------- ui helpers ----------
def get_profile_exists(user_id):
    res = auth.supabase.table(var.table_user_profile).select("*").eq(var.col_user_id, user_id).execute()
    return len(res.data) > 0


def hinge_sync_flow():
    """Run uploader; if it returns truthy, rerun."""
    done = uploader()
    if done:
        st.rerun()
...
def signout_dialog():
    ...
    with c1:
        if st.button("Cancel", width="stretch"):
            ...
            st.rerun()
    with c2:
        if st.button("Yes, sign out", type="primary", width="stretch"):
            auth.sign_out()
            st.session_state.show_signout = False
            st.rerun()
...
def delete_data_dialog():
    ...
    with st.expander("Delete data & account"):
        st.warning("This will permanently delete your account and all associated data.")
        confirm_text = st.text_input('Type "delete" to confirm', key="delete_account_confirm_text")
    ...
    def _dialog():
        ...
        with tab_table:
            st.markdown(
                ...
                **Searching**
                - Click into the table first, then use Ctrl/Cmd+F.
                - Or use the magnifying glass in the toolbar.
                """
            )
            df_demo = pd.DataFrame(
                {"Name": ["Ava", "Noah", "Mia", "Liam"], "Score": [72, 95, 88, 60], "Group": ["North", "North", "South", "South"]}
            )
            st.dataframe(df_demo, use_container_width=True, hide_index=True)
        ...
        with tab_plotly:
            st.markdown(
                ...
                **Modebar**
                Hover the chart to see zoom/pan/reset/download tools.  
                [Plotly modebar guide](https://plotly.com/chart-studio-help/getting-to-know-the-plotly-modebar/)
            ...
            df_bar = pd.DataFrame(
                {"Week": ["W1", "W1", "W1", "W2", "W2", "W2", "W3", "W3", "W3"], "Type": ["Like", "Match", "Message"] * 3, "Count": [30, 12, 5, 25, 14, 7, 18, 10, 9]}
            )
            fig_bar = px.bar(df_bar, x="Week", y="Count", color="Type", barmode="stack")
            st.plotly_chart(fig_bar, use_container_width=True)
        ...
        with tab_box:
            st.markdown(
                ...
                - Median = 50th
                - Q3 = 75th
                Whiskers show the main range; far points are outliers.
                """
            )
            np.random.seed(42)
            core = np.random.normal(loc=50, scale=7, size=220)
            values = np.concatenate([core, [10, 95]])
            df_box = pd.DataFrame({"Value": values})
            fig_box = px.box(df_box, x="Value", points="outliers", orientation="h")
            st.plotly_chart(fig_box, use_container_width=True)
...
@st.dialog("Send feedback")
def feedback_dialog():
    st.markdown("[View the GitHub repo](https://github.com/Shantoun/Unhinged/tree/main)")
...
# ---------- main app ----------
def run_app():
    # IMPORTANT: set page config ONCE, before any other st.* output
    ...
    # ---------------- logged in ----------------
    if user_id:
        has_profile = get_profile_exists(user_id)

        # ---- First-time sync gate ----
        if not has_profile:
            st.header("Sync Your Hinge Data")
            hinge_sync_flow()
            return
        ...
        with st.sidebar:
            ...
            if st.button("Upload More Data", width="stretch"):
                hinge_sync_dialog()

            if st.button("Sign Out", width="stretch"):
                st.session_state.show_signout = True

            if st.button("Delete My Data", width="stretch", key="open_delete"):
                st.session_state.show_delete_dialog = True
        ...
        tab1, tab2, tab3, tab4, tab5, tab6 = st.tabs(
            [
                var.tab_engagement_funnel,
                var.tab_engagement_over_time,
                var.tab_outbound_timing,
                var.tab_drivers,
                var.tab_subscriptions,
                var.tab_distribution,
            ]
        )
        ...
        with tab1:
            ...
            join_likes_comments = st.checkbox("Join comments & likes sent", key="join_likes_comments_tab1", on_change=sync_from_tab1)
            c1, c2 = st.columns(2)
            convo_min_mins = c1.number_input(
                "Minimum conversation duration (min)",
                min_value=0,
                step=1,
                width="stretch",
                key="convo_min_mins_tab1",
                on_change=sync_from_tab1,
                help="Sets the minimum duration required for an interaction to count as a conversation.",
            )
            convo_min_messages = c2.number_input(
                "Minimum messages per conversation",
                min_value=0,
                step=1,
                width="stretch",
                key="convo_min_messages_tab1",
                on_change=sync_from_tab1,
                help="Sets the minimum number of messages required to count as a conversation.",
            )

            sankey_data = ds.sankey_data(
                engagements,
                min_messages=convo_min_messages,
                min_minutes=convo_min_mins,
                join_comments_and_likes_sent=join_likes_comments,
            )
            fig_sankey = viz.sankey(sankey_data, len(engagements))
            st.plotly_chart(fig_sankey, width="stretch")
        ...
        with tab2:
            ...
            use_like_time = st.checkbox(
                "Use like timestamp instead of event timestamp",
                help="""
                    Controls which timestamp is used to place events into time buckets.
                    When enabled, events are grouped by when the like was sent or received.
                    When disabled, events are grouped by when the event itself occurred (e.g., match, message).
            ...
            convo_min_messages = c2.number_input(
                ...
                step=1,
                width="stretch",
                key="convo_min_messages_tab2",
                on_change=sync_from_tab2,
            )

            engagements_over_time = ds.events_over_time_df(
                engagements,
                min_messages=convo_min_messages,
                min_minutes=convo_min_mins,
                join_comments_and_likes_sent=join_likes_comments,
                use_like_timestamp=use_like_time,
            )
        ...
        with tab3:
            ...
            st.caption(
                "The score used below is more reliable than a raw match rate. It reduces the impact of small samples so results reflect real patterns."
            )
            ...
            out = pd.DataFrame({"Peak Times": best, "Off Times": worst})
            out.index = [""] * len(out)
            st.table(out, border="horizontal")
            ...
            def rename_columns(df):
                ...
                return df.rename(columns={k: v for k, v in rename_map.items() if k in df.columns}).reset_index(drop=True)

            time_table = rename_columns(time_table).sort_values(["Score", "Comments & Likes"], ascending=[False, True])
            day_table = rename_columns(day_table).sort_values(["Score", "Comments & Likes"], ascending=[False, True])
            day_time_table = rename_columns(day_time_table)
            ...
            for df in (day_table, time_table, day_time_table):
                df["Match Rate"] = df["Match Rate"].map(fmt_pct)
                df["Score"] = df["Score"].apply(lambda x: 0 if x == 0 else round(x, 1))

            with st.expander("View as data"):
                st.dataframe(time_table.set_index("Time Slot"))
                st.dataframe(day_table.set_index("Day of Week"))
                st.dataframe(day_time_table, hide_index=True)
        ...
        with tab6:
            ...
            with st.expander("View as data"):
                ...
                df_message_durations["Minutes"] = df_message_durations["Minutes"].apply(lambda lst: sorted(fmt_1dp(v) for v in lst))
                df_messages_per_session["Messages"] = df_messages_per_session["Messages"].apply(lambda lst: sorted(fmt_int(v) for v in lst))
                df_like_to_match_time["Minutes"] = df_like_to_match_time["Minutes"].apply(lambda lst: sorted(fmt_1dp(v) for v in lst))
    ...
    # ---------------- logged out ----------------
    st.set_page_config  # (intentionally not calling again)
