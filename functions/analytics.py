...
def sankey(sankey_df, numb_of_engagements):
    ...
    df["s_idx"] = df["Source"].map(label_to_idx).astype(int)
    df["t_idx"] = df["Target"].map(label_to_idx).astype(int)
    ...
    for s_i, t_i, v in zip(df["s_idx"], df["t_idx"], df["Value"]):
        ...
        txt = f"{s_label} → {t_label}<br>{pct:.1%} of {s_label}: {int(v)}"
    ...
    for t_i in range(len(labels)):
        ...
        for _, r in rows.iterrows():
            ...
            v = float(r["Value"])
            pct = pct_of_source(v, s_i)
            lines.append(f"{pct:.1%} of {labels[s_i]}: {int(v)}")
